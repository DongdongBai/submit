...
class Game:
	def __init__(self, path=-1, cards = 3, bidmaximum=5, savepath="", save=False):
		...
		for h in range(self.numHists):
			if self.isTerminal[h]:
				...
				self.reward[h] = np.array([rew, -rew])

		self.transObs = list(map(lambda _u: np.ones(self.nactsOnHist[_u]), range(self.numHists)))
		self.rewObs = list(map(lambda _u: np.ones(2), range(self.numHists)))

		if save:
			if savepath == "":
				savepath = "games/leduc_"+str(cards)+"_"+str(bidmaximum)+".npz"
			np.savez(savepath,  numHists= self.numHists, isTerminal= self.isTerminal, reward= self.reward, 
	...
	def resample(self):
		trans = []
		rews = []
		for i in range(self.numHists):
			if self.playerOfHist[i] == 2:
				trans.append(np.random.dirichlet(self.transObs[i]))
			else:
				trans.append(0)
			if self.isTerminal[i]:
				a = np.random.dirichlet(self.rewObs[i])
	...
	def simulate(self, h):
		p = self.playerOfHist[h]
		if self.isTerminal[h] == True:
			ret = np.random.choice(2, p=[(self.reward[h][0] + 1)/2, 1 - (self.reward[h][0] + 1) / 2 ])
			self.rewObs[h][ret] += 1

			return np.array([1 - ret * 2, ret * 2 - 1])
		if p == 2:
			ret = np.random.choice(self.nactsOnHist[h], p=self.chanceprob[h])
			self.transObs[h][ret] += 1

			return self.histSucc[h][ret]
			...
			for i in range(x):
				for j in range(y):
					...
					if win == 0:
						...
						self.reward[int(histids[i][j])] = (bids[1]+1, -bids[1]-1)
					elif win == -1:
						self.reward[int(histids[i][j])] = (-bids[0]-2, bids[0]+2)
